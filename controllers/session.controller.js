...
export async function handleMessageSSE(req, res) {
  try {
    ...
    // Event emitter callback for mode updates
    const emitMode = (mode) => {
      res.write(`event: mode\n`);
      res.write(`data: ${JSON.stringify({ mode })}\n\n`);
    };

    // Process the message with mode callback
    const agentResponse = await agentService.handleMessage(session, message, emitMode);
